...
class SeatLockingService {
     ...
     */
    static async lockSeats(seatIds, userId, eventId, duration = this.LOCK_EXPIRY) {
        try {
            ...
            logger_1.logger.error('Error locking seats', { error, seatIds, userId, eventId });
     ...
     * @returns Object with success status and message
     */
    static async releaseSeats(seatIds, userId, eventId) {
        try {
            if (!seatIds || seatIds.length === 0) {
            ...
            logger_1.logger.error('Error releasing seats', { error, seatIds, userId, eventId });
     ...
     */
    static async checkSeatsLocked(seatIds, eventId) {
        try {
            const lockedSeats = [];
            const lockedBy = {};
            for (const seatId of seatIds) {
            ...
            logger_1.logger.error('Error checking seat locks', { error, seatIds, eventId });
     ...
     * @returns Object with success status and message
     */
    static async extendLock(seatIds, userId, eventId, duration = this.LOCK_EXPIRY) {
        try {
            if (!seatIds || seatIds.length === 0) {
            ...
            logger_1.logger.error('Error extending seat locks', { error, seatIds, userId, eventId });
     ...
     * @returns Object with success status and message
     */
    static async confirmSeats(seatIds, userId, eventId, bookingId) {
        try {
            ...
            logger_1.logger.error('Error confirming seats', { error, seatIds, userId, eventId, bookingId });
