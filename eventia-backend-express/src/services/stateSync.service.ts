...
export class StateSyncService {
  ...
  private static activeLocks = new Map<string, {userId: string, expiresAt: Date}>();
   ...
   */
  static async acquireSeatLock(seatId: string, userId: string, durationSeconds = 300): Promise<boolean> {
   ...
   */
  static async releaseSeatLock(seatId: string, userId: string): Promise<boolean> {
    // Check if lock exists and belongs to user
    const existingLock = this.activeLocks.get(seatId);
    if (existingLock && existingLock.userId !== userId) {
      throw new ApiError(403, 'Cannot release lock owned by another user', 'FORBIDDEN');
    }
   ...
   */
  static async checkSeatLock(seatId: string): Promise<{userId: string, expiresAt: Date} | null> {
   ...
   */
  static notifySeatStatusChange(seatId: string, status: string, userId?: string): void {
    if (!this.io) return;
   ...
   */
  static notifyBookingStatusChange(bookingId: string, status: string): void {
    if (!this.io) return;
