...
export class BookingControllerV1 {
  ...
  static createBooking = asyncHandler(async (req: Request, res: Response) => {
      ...
      const ticketPromises = seats.map(seat =>
        prismaClient.ticket.create({
          data: {
            ...
            bookingId: newBooking.id,
            seatId: seat.id,
            code: `TKT-${Math.floor(Math.random() * 1000000)}`,
            status: 'ACTIVE',
            ticketCategoryId: seat.ticketCategoryId || 'unknown',
            price: seat.price || 0
          }
        })
      );
    ...
    return ApiResponse.success(res, 201, 'Booking created successfully', bookingResult);
  });
  ...
  static getUserBookings = asyncHandler(async (req: Request, res: Response) => {
    ...
    return ApiResponse.success(res, 200, 'User bookings fetched successfully', {
  ...
  static cancelBooking = asyncHandler(async (req: Request, res: Response) => {
    ...
    return ApiResponse.success(res, 200, 'Booking cancelled successfully', { id });
  });

  /**
   * Get all bookings (Admin only)
   */
  static getAllBookings = asyncHandler(async (req: Request, res: Response) => {
    // Check if admin
    ...
    return ApiResponse.success(res, 200, 'Bookings fetched successfully', {
      bookings,
      pagination: {
        total,
        page: Number(page),
