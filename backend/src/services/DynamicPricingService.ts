...
// Use Prisma-generated type for PricingRule
type PricingRule = {
  id: string;
  name: string;
...
export default class DynamicPricingService {
    ...
    try {
      ...
      for (const rule of sortedRules) {
        ...
        if (rule.adjustmentType === 'PERCENTAGE') {
          adjustmentAmount = (finalPrice * (Number(rule.adjustmentValue) / 100));
        } else { // FIXED
          adjustmentAmount = Number(rule.adjustmentValue);
        }
  ...
  private async evaluateRuleConditions(
    ...
    try {
      const { type, conditions: rawConditions } = rule;
