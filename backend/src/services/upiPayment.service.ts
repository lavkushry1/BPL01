...
export const generateUpiLink = (amount: number, referenceId: string, options: any = {}) => {
    ...
    const params = {
        pa: config.upiPayment.merchantUpiId, // VPA (UPI ID) of the merchant
        pn: config.upiPayment.merchantName,  // Merchant name
...
export const validateWebhookSignature = (payload: any, signature: string): boolean => {
    try {
        ...
        const expectedSignature = crypto
            .createHmac('sha256', config.upiPayment.webhookSecret)
