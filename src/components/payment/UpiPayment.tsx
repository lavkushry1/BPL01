...
const UpiPayment = ({ bookingId, amount, onPaymentSuccess, onUtrSubmit, customerInfo }: UpiPaymentProps) => {
    ...
    // Create a fallback QR code without relying on the backend
    const createFallbackQrCode = (vpaId: string) => {
        try {
            if (!vpaId || typeof vpaId !== 'string') {
                vpaId = 'eventia@okicici'; // Default fallback if vpaId is invalid
            }

            setUpiId(vpaId);

            // Generate a UPI deep link directly
            const fallbackUpiUrl = `upi://pay?pa=${vpaId}&pn=EventiaTickets&am=${amount}&tr=${bookingId}&cu=INR`;

            // Make sure we have a valid string for QR code
            if (fallbackUpiUrl && typeof fallbackUpiUrl === 'string') {
                setQrValue(fallbackUpiUrl);
            } else {
                throw new Error('Invalid UPI URL generated');
            }
        } catch (error) {
            console.error("Error in fallback QR code generation:", error);
            // Ultimate fallback with hardcoded values
            const lastResortUrl = `upi://pay?pa=eventia@okicici&pn=EventiaTickets&am=${amount}&cu=INR`;
            setQrValue(lastResortUrl);
            setQrLoadError(true);
        }
    };
            ...
            try {
                ...
                if (response.data?.data?.upi?.vpa) {
                    ...
                    try {
                        ...
                        if (upiUrl && typeof upiUrl === 'string') {
                            setQrValue(upiUrl);
                            setQrCodeUrl(qrUrl);
                        } else {
                            throw new Error('Invalid UPI URL format received');
                        }
                    } catch (error) {
                        console.error('Failed to generate QR code:', error);
                        // Fall back to a locally generated QR code
                        createFallbackQrCode(response.data.data.upi.vpa);
                    }
                } else {
                    ...
                    createFallbackQrCode('eventia@okicici');
                }
            } catch (error) {
                ...
                // Try to create a fallback QR code
                try {
                    createFallbackQrCode('eventia@okicici');
                } catch (fallbackError) {
    ...
    const refreshQrCode = async () => {
        ...
        try {
            // Generate a client-side QR code first as a reliable fallback
            const fallbackUpiUrl = `upi://pay?pa=eventia@okicici&pn=EventiaTickets&am=${amount}&tr=${bookingId}&cu=INR`;
            setQrValue(fallbackUpiUrl); // Set this immediately so we have something to show

            // Now try to get official UPI settings from the API
            const response = await paymentApi.getPaymentSettings();
            ...
            if (response?.data?.data?.upi?.vpa) {
                ...
                try {
                    // Try to generate QR code from backend
                    const { qrUrl, upiUrl } = await paymentApi.generateUpiQrCode(amount, bookingId);
                    if (upiUrl && typeof upiUrl === 'string') {
                        setQrValue(upiUrl);
                        setQrCodeUrl(qrUrl);
                    ...
                    console.error('Error generating QR code from API:', qrError);
                    // Keep using our initial fallback - we already set it above
                    const merchantVpa = response.data.data.upi.vpa;
                    const betterUpiUrl = `upi://pay?pa=${merchantVpa}&pn=EventiaTickets&am=${amount}&tr=${bookingId}&cu=INR`;
                    setQrValue(betterUpiUrl);
                }
            }
        } catch (error) {
            console.error('Error refreshing QR code:', error);
            // We already set the fallback QR code above, so no need to do anything here
        } finally {
            setIsLoading(false);
        }
    };
                ...
                <div className="bg-card border rounded-lg p-4 flex flex-col items-center">
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        {isLoading ? (
                            ...
                            <SimpleQrCode
                                value={qrValue || `upi://pay?pa=${upiId || 'eventia@okicici'}&pn=EventiaTickets&am=${amount}&tr=${bookingId}&cu=INR`}
                                size={200}
                                className="my-4"
                                fallbackUpi={upiId || 'eventia@okicici'}
                                paymentDetails={{
                                    upiId: upiId || 'eventia@okicici',
                                    amount: amount,
                                    description: bookingId ? `Booking: ${bookingId}` : 'Ticket payment'
                                }}
                        ...
                        <div className="w-full mt-2 space-y-2">
                            <div className="flex items-center justify-between p-2 bg-muted rounded">
                                <div className="text-sm font-medium overflow-hidden text-ellipsis">{upiId || 'eventia@okicici'}</div>
                                <Button
                                    variant="ghost"
                                    size="icon"
                                    onClick={() => {
                                        navigator.clipboard.writeText(upiId || 'eventia@okicici');
                                        toast({
                                            title: t('payment.copied', 'Copied!'),
                                            description: t('payment.upiIdCopied', 'UPI ID copied to clipboard'),
                                        });
                                    }}
                                >
